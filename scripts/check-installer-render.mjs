...
evopi_read_terminal_size() {
	evopi_screen_cols="$evopi_test_cols"
	evopi_screen_rows="$evopi_test_rows"
}
...
print_render_meta() {
	...
	printf '__META__ %s cols=%s rows=%s layout_show_logo=%s lab_width=%s render_lab_width=%s compact=%s visible=%s content_height=%s\\n' \\
		"$label" "$evopi_screen_cols" "$evopi_screen_rows" "$evopi_screen_layout_show_logo" \\
		"$evopi_screen_layout_lab_width" "$evopi_screen_render_lab_width" "$evopi_screen_compact" "$visible" "$content_height"
}

render_case() {
	evopi_screen_title="Installing evopi"
	evopi_screen_detail="Fetching the verified package."
	evopi_screen_question=
	evopi_screen_frame=1
	evopi_screen_cols="$1"
	evopi_screen_rows="$2"
...
screen_case() {
	...
	evopi_screen "Installing evopi" "Installing evopi" "Fetching the verified package." ""
...
Finalizing npm install."
	for progress_frame in 1 24 25 48 49 200; do
		evopi_animation_frame="$progress_frame"
		printf '__PROGRESS__ %s\t%s\t%s\\n' "$progress_frame" "$(evopi_animation_status "Installing evopi" "$progress_details" static)" "$(evopi_animation_detail "$progress_details")"
	done
}
...
const tempDir = mkdtempSync(join(tmpdir(), "evopi-installer-render-"));
...
function assertInstallerProgress(progress) {
	...
	for (const [index, expectedDetail] of expectedDetails.entries()) {
		...
		check(
			progress[index].status === "Installing evopi...",
