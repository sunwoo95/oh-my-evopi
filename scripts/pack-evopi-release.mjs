...
const defaultBaseUrl = process.env.EVOPI_DOWNLOAD_BASE_URL;
const publicPackageName = process.env.EVOPI_PACKAGE_NAME || "evopi";
const publicCommandName = process.env.EVOPI_CMD || "evopi";
const releaseChannels = new Set(["stable", "beta"]);

const releasePackages = [
	// Internal workspace deps must ship as tarballs too: their references in
	// dependent packages get rewritten to release tarball URLs, so any omission
	// leaks to the public npm registry and 404s at `npm install -g`.
	// natives-loader ← hashline ← coding-agent (transitive chain), so both are
...
function parseArgs(args) {
	...
	if (!parsed.baseUrl) {
		throw new Error("--base-url or EVOPI_DOWNLOAD_BASE_URL is required");
	}
...
function printHelp() {
	console.log(`Usage: node scripts/pack-evopi-release.mjs --base-url url [--channel stable|beta] [--version x.y.z] [--out-dir path]

Creates private npm tarballs for R2 distribution:

  <out-dir>/artifacts/evopi-<version>.tgz
  <out-dir>/artifacts/evopi-ai-<version>.tgz
  <out-dir>/artifacts/evopi-core-<version>.tgz
  <out-dir>/artifacts/evopi-tui-<version>.tgz
...
function main() {
	...
	const releaseVersion = args.version || normalizeVersion(process.env.EVOPI_VERSION || cliPackage.version);
