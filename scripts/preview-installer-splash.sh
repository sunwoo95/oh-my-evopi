...
frames="${1:-${EVOPI_SPLASH_PREVIEW_FRAMES:-300}}"
case "$frames" in
	""|*[!0-9]*)
...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/evopi-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	evopi_screen "Installing evopi" "Downloading evopi$(evopi_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
