...
import { enclosingBlockBoundaries, type NodeSpan, nodeChainAt } from "./native.js";
...
export function nodeChain(lines: readonly string[], path: string, line: number): readonly NodeSpan[] {
	...
	const key = `${hashKey(text)}:${text.length}:${path}:${line}`;
...
export function enclosingBoundaries(
	...
	const text = lines.join("\n");
	const key = `${hashKey(text)}:${text.length}:${path}:${startLine}:${endLine}`;
...
export function parsesCleanly(path: string | undefined, text: string): boolean {
	...
	const key = `${hashKey(text)}:${text.length}:${path}`;
