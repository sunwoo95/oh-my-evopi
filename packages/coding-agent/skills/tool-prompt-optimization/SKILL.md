...
`scripts/probe.ts`: `@evopi/pi-ai` `completeSimple`; production-matching model/auth/provider behavior.

```bash
bun .evopi/agent/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .evopi/agent/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@evopi/pi-ai"; // or toolWireSchema(tool)
