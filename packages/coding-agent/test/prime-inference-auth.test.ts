...
describe("Prime Inference auth", () => {
	...
	it("checks evopi trace access with Prime whoami permissions", async () => {
		const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
			expect(getUrl(input)).toBe("https://prime-api.example/api/v1/user/whoami");
			expect(init?.method).toBe("GET");
			expect(getAuthorization(init)).toBe("Bearer prime-key");
	...
	it("validates imported Prime CLI trace credentials against EVOPI_TRACES_BASE_URL", async () => {
	...
	it("requests agent trace scope during evopi trace browser login", async () => {
		process.env.EVOPI_TRACES_BASE_URL = "https://prime-api.example/api/v1";
		writeFileSync(
			configPath,
			JSON.stringify({
				base_url: "https://prime-api.example",
