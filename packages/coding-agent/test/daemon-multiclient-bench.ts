...
const streamCorpus = repeatedText(STREAM_CHARS, "evopi daemon multi-client streaming benchmark. ");
		...
		for (let offset = 0, sequence = 1; offset < streamCorpus.length; sequence++) {
			...
			const compact = createCompactAssistantDelta({
				...
				meta: {
					...
					protocol: { name: "evopi.daemon", version: 2 },
					activeSessionId: "active-benchmark",
					sequence,
...
function printResults(
	...
	console.log("evopi daemon multi-client benchmark");
	console.log(details);
	console.table(
		results.map((result) => ({
...
function printSessionLoad(sessionFile: string, result: SessionLoadResult): void {
	console.log("evopi real-session benchmark");
