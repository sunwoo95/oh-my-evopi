...
describe("InteractiveMode MCP command", () => {
	...
	test("keeps OAuth guidance accurate for legacy UI service hosts", async () => {
		...
		expect(normalizeRenderedOutput(fakeThis.chatContainer)).toContain(
			"Restart evopi, then run /mcp login remote to connect.",
		);
	});
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		...
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [{ path: path.join(home, ".evopi", "agent", "AGENTS.md") }, { path: path.join(cwd, "AGENTS.md") }],
		});
		...
		expect(output).toContain("~/.evopi/agent/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.evopi/agent/AGENTS.md");
		expect(output).toContain("~/Development/pi-mono/AGENTS.md");
		expect(output).not.toContain("~/.evopi/agent/AGENTS.md, AGENTS.md");
	});
