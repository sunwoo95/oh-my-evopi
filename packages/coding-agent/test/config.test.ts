...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	const scopeDir = join(root, "@evopi");
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@evopi+pi-coding-agent@0.67.68\\node_modules\\@evopi\\pi-coding-agent\\dist\\cli.js",
		);

		expect(detectInstallMethod()).toBe("pnpm");
		expect(getUpdateInstruction("@evopi/pi-coding-agent")).toBe("Run: pnpm install -g @evopi/pi-coding-agent");
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@evopi/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@evopi/pi-coding-agent")).toBe(
			"Update @evopi/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("leaves Homebrew installs under Homebrew ownership", () => {
		...
		expect(getSelfUpdateCommand("@evopi/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@evopi/pi-coding-agent")).toBe("Update with: brew upgrade evopi");
		expect(getUpdateInstruction("@evopi/pi-coding-agent")).toBe("Update with: brew upgrade evopi");
	});

	test("self-updates npm installs from custom prefixes", () => {
		const { prefix } = createNpmPrefixInstall();

		const command = getSelfUpdateCommand("@evopi/pi-coding-agent");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "@evopi/pi-coding-agent"],
			display: `npm --prefix ${prefix} install -g @evopi/pi-coding-agent`,
		});
	});
	...
	test("self-updates tarball specs without uninstalling the same logical package first", () => {
		...
		const command = getSelfUpdateCommand("@evopi/pi-coding-agent", undefined, tarballUrl);
	...
	test("self-updates renamed tarball packages by uninstalling the old package after install", () => {
		...
		const command = getSelfUpdateCommand("@evopi/pi-coding-agent", undefined, tarballUrl, "prime-agent");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", tarballUrl],
			display: `npm --prefix ${prefix} install -g ${tarballUrl} && npm --prefix ${prefix} uninstall -g @evopi/pi-coding-agent`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", tarballUrl],
				...
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@evopi/pi-coding-agent"],
					display: `npm --prefix ${prefix} uninstall -g @evopi/pi-coding-agent`,
				},
			],
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@evopi/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "@evopi/pi-coding-agent"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g @evopi/pi-coding-agent`);
	});

	test("does not infer Windows npm custom prefixes from package paths", () => {
		const packageDir = "C:\\Users\\Admin\\npm prefix\\node_modules\\@evopi\\pi-coding-agent";
		process.env.PI_PACKAGE_DIR = packageDir;
		setExecPath(`${packageDir}\\dist\\cli.js`);

		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@evopi/pi-coding-agent")).toBe("Run: npm install -g @evopi/pi-coding-agent");
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "@evopi/pi-coding-agent"],
			display: "bun install -g @evopi/pi-coding-agent",
		});
	});
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@evopi/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@evopi/pi-coding-agent")).toContain(
...
describe("session paths", () => {
	test("uses the short app-prefixed session dir env var", () => {
		expect(ENV_SESSION_DIR).toBe("EVOPI_SESSION_DIR");
	});
