...
vi.mock("../src/package-manager-cli.js", () => ({
	...
	isSelfUpdateSource: (source: string) => source === "self" || source === "pi" || source === "evopi",
}));
...
describe("public command routing", () => {
	...
	it("rejects extra attach operands", async () => {
		...
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("evopi attach <agent>"));
	});
	...
	it("separates evopi updates from package updates", async () => {
	...
	it("gives legacy update targets explicit migration guidance", async () => {
		for (const target of ["self", "--self", "evopi"]) {
			await handlePublicCommand(["update", target]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "evopi update [--force]"'));
	});
	...
	it("directs legacy package-update forms to the package command", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "evopi package update [source]"'));
	});
	...
	it("rejects self-update aliases on the package update path", async () => {
		for (const source of ["self", "pi", "evopi"]) {
			await handlePublicCommand(["package", "update", source]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "evopi update"'));
	});
	...
	it("directs package uninstall to package remove", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "evopi package remove"'));
	...
	it("rejects operands for package list", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("evopi package list"));
	});
	...
	it("rejects the old daemon hierarchy with migration guidance", async () => {
		...
		expect(process.exitCode).toBe(1);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Run "evopi help"'));
	});

	it("shows migration guidance when help targets removed commands", async () => {
		const cases: Array<[path: string[], hint: string]> = [
			[["daemon"], 'Run "evopi help"'],
			[["install"], 'Use "evopi package install"'],
			[["remove"], 'Use "evopi package remove"'],
			[["uninstall"], 'Use "evopi package remove"'],
			[["manage"], 'Use "evopi agents"'],
			[["app", "update"], 'Use "evopi update"'],
		];
	...
	it("shows command help when options precede the help flag", async () => {
		...
		expect(console.log).toHaveBeenNthCalledWith(1, expect.stringContaining("evopi list [--all] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(2, expect.stringContaining("evopi doctor [--fix] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(3, expect.stringContaining("evopi package install <source>"));
