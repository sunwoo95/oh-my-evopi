...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".evopi", "agent", "prompts");
			...
			const projectSkillDir = join(cwd, ".evopi", "agent", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".evopi", "agent", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".evopi", "agent", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".evopi", "agent"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".evopi", "agent", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".evopi", "agent", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".evopi", "agent", "extensions");
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.evopi", async () => {
			const piDir = join(cwd, ".evopi", "agent");
...
Content`,
	...
	describe("bundled skills", () => {
		...
		it("should let a project skill override the bundled websearch skill", async () => {
			const projectSkillDir = join(cwd, ".evopi", "agent", "skills", "websearch");
			mkdirSync(projectSkillDir, { recursive: true });
			writeFileSync(
