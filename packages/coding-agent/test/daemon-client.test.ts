...
function emitHello(
	...
	socket.emit(
		...
		`${JSON.stringify({
			...
			protocol: { name: "evopi.daemon", version },
			schemaRevision,
			appVersion: "9.9.9",
			clientId: "client-1",
			serverCapabilities,
...
describe("DaemonClient", () => {
	...
	it("allows connect retry after the socket emits an error before connecting", async () => {
		...
		expect(firstError.message).toContain("Failed to connect to the evopi daemon: initial connect failed.");
		...
		await expect(secondAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Failed to connect to the evopi daemon: retry reached socket."),
		});
	});
	...
	it("allows connect retry after the initial connection times out", async () => {
		...
		const timeoutRejection = expect(firstAttempt).resolves.toMatchObject({
			message: expect.stringContaining("Timed out after 5ms connecting to the evopi daemon."),
		});
	...
	it("captures the daemon hello greeting for version checks", async () => {
		...
		const hello = {
			type: "daemon_hello",
			socketPath: "/tmp/prime-agent.sock",
			protocol: { name: "evopi.daemon", version: 1 },
			appVersion: "9.9.9",
			clientId: "client-1",
			serverCapabilities: [],
		};
	...
	it("serializes activeSessionId for session commands", async () => {
		...
		expect(envelope).toMatchObject({
			...
			protocol: { name: "evopi.daemon", version: DAEMON_PROTOCOL_VERSION },
	...
	it("includes command, socket, and log context when a request is made while disconnected", async () => {
		...
		await expect(request).rejects.toMatchObject({
			message: expect.stringContaining(
				'Cannot send daemon command "list" because the evopi daemon is not connected.',
			),
		});
		await expect(request).rejects.toMatchObject({
	...
	it("notifies listeners when a connected daemon socket closes", async () => {
		...
		expect(closed[0]?.message).toContain("Connection to the evopi daemon closed.");
