...
describe("SettingsManager", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(join(projectDir, ".evopi", "agent"), { recursive: true });
	});
	...
	describe("evo (M11) → autoRefine mapping", () => {
		afterEach(() => {
			delete process.env.EVOPI_EVO;
		});

		it("evo unset → prime default (autoRefine enabled), resolveEvoEnabled undefined", () => {
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.resolveEvoEnabled()).toBeUndefined();
			expect(manager.getAutoRefineSettings().enabled).toBe(true);
		});

		it("evo.enabled=false setting → pure control arm (autoRefine disabled)", () => {
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ evo: { enabled: false } }));
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.resolveEvoEnabled()).toBe(false);
			expect(manager.getAutoRefineSettings().enabled).toBe(false);
		});

		it("evo.enabled=true setting → grounded arm (autoRefine stays enabled)", () => {
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ evo: { enabled: true } }));
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.resolveEvoEnabled()).toBe(true);
			expect(manager.getAutoRefineSettings().enabled).toBe(true);
		});

		it("EVOPI_EVO env overrides the evo.enabled setting", () => {
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ evo: { enabled: true } }));
			const manager = SettingsManager.create(projectDir, agentDir);
			process.env.EVOPI_EVO = "off";
			expect(manager.resolveEvoEnabled()).toBe(false);
			expect(manager.getAutoRefineSettings().enabled).toBe(false);
			process.env.EVOPI_EVO = "on";
			expect(manager.resolveEvoEnabled()).toBe(true);
			expect(manager.getAutoRefineSettings().enabled).toBe(true);
		});

		it("explicit autoRefine opt-out is honored even when evo is on", () => {
			writeFileSync(
				join(agentDir, "settings.json"),
				JSON.stringify({ evo: { enabled: true }, autoRefine: { enabled: false } }),
			);
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getAutoRefineSettings().enabled).toBe(false);
		});
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".evopi", "agent", "settings.json");
		...
		it("should report a new project error when saving after the load error was drained", async () => {
			const settingsPath = join(projectDir, ".evopi", "agent", "settings.json");
		...
		it("drains only the requested scope", () => {
			...
			writeFileSync(join(projectDir, ".evopi", "agent", "settings.json"), "{ invalid project json");
			const manager = SettingsManager.create(projectDir, agentDir);
	...
	describe("project settings directory creation", () => {
		it("should not create .evopi folder when only reading project settings", () => {
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			rmSync(join(projectDir, ".evopi", "agent"), { recursive: true });

			const manager = SettingsManager.create(projectDir, agentDir);

			expect(existsSync(join(projectDir, ".evopi", "agent"))).toBe(false);

			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .evopi folder when writing project settings", async () => {
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			expect(existsSync(join(projectDir, ".evopi", "agent"))).toBe(true);

			expect(existsSync(join(projectDir, ".evopi", "agent", "settings.json"))).toBe(true);
		});
	});
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(
				join(projectDir, ".evopi", "agent", "settings.json"),
