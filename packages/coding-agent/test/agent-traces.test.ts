...
import { EVOPI_TRACES_PROVIDER_ID, PRIME_INFERENCE_PROVIDER_ID } from "../src/core/prime-inference-auth.js";
...
describe("agent trace upload", () => {
	...
	beforeEach(() => {
		...
		originalTraceApiKey = process.env.EVOPI_TRACES_API_KEY;
		originalPrimeApiKey = process.env.PRIME_API_KEY;
		originalTraceBaseUrl = process.env.EVOPI_TRACES_BASE_URL;
		originalPrimeBaseUrl = process.env.PRIME_API_BASE_URL;
		delete process.env.EVOPI_TRACES_API_KEY;
		delete process.env.PRIME_API_KEY;
		delete process.env.EVOPI_TRACES_BASE_URL;
	...
	afterEach(() => {
		...
		if (originalTraceApiKey === undefined) {
			...
			process.env.EVOPI_TRACES_API_KEY = originalTraceApiKey;
		}
		...
		if (originalTraceBaseUrl === undefined) {
			...
			process.env.EVOPI_TRACES_BASE_URL = originalTraceBaseUrl;
		}
	...
	it("does not upload when trace sharing is disabled", async () => {
		...
		const result = await uploadAgentTraceFile({
			...
			authStorage: AuthStorage.inMemory({
				[EVOPI_TRACES_PROVIDER_ID]: { type: "api_key", key: "trace-key" },
			}),
	...
	it("uses EVOPI_TRACES_BASE_URL for trace API overrides", async () => {
		const sessionManager = writeSession(tempDir, join(tempDir, "sessions"), "override-session");
		process.env.EVOPI_TRACES_BASE_URL = "https://trace-api.example/api/v1";
