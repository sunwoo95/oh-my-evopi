...
describe("package commands", () => {
	...
	beforeEach(() => {
		...
		originalPrimeAgentDownloadBaseUrl = process.env.EVOPI_DOWNLOAD_BASE_URL;
	...
	afterEach(() => {
		...
		restoreEnv("EVOPI_DOWNLOAD_BASE_URL", originalPrimeAgentDownloadBaseUrl);
	...
	it("uses global npmCommand and the release manifest install spec for forced self updates", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@evopi", "pi-coding-agent");
...
`,
	...
	it("installs the evopi tarball from the update manifest during self-update", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@evopi", "pi-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
...
`,
		...
		process.env.PI_PACKAGE_DIR = selfPackageDir;
		process.env.EVOPI_DOWNLOAD_BASE_URL = baseUrl;
		Object.defineProperty(process, "execPath", {
			value: join(selfPackageDir, "dist", "cli.js"),
			configurable: true,
		});
	...
	it("does not self-update when the same-version manifest uses the evopi package alias", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@evopi", "pi-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
