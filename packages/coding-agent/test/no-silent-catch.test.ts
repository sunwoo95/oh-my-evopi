...
describe("no silent catch blocks", () => {
	test("every empty catch in packages/*/src carries an explanatory comment", () => {
		...
		expect(
			offenders,
			`Empty catch blocks swallow errors silently. Log the error (getLogger from @evopi/pi-ai) or add a comment inside the block explaining why ignoring it is safe:\n${offenders.join("\n")}`,
		).toEqual([]);
	});
});
