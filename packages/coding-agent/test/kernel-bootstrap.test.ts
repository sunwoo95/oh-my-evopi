...
describe("kernel bootstrap", () => {
	beforeEach(async () => {
		...
		delete process.env.EVOPI_KERNEL_PYTHON;
		delete process.env.EVOPI_KERNEL_VENV;
	...
	it("returns the configured kernel venv directory", () => {
		...
		process.env.EVOPI_KERNEL_VENV = venv;

		expect(getKernelVenvDir()).toBe(venv);
	});

	it("bootstraps a missing venv with uv, evopi-runtime, and default extra packages", async () => {
		...
		expect(log).toContain("evopi-runtime");
...
`,
	...
	it("rejects EVOPI_KERNEL_PYTHON missing default extra packages", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects EVOPI_KERNEL_PYTHON with a stale rlm runtime", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, ["dill"]);
		process.env.EVOPI_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).rejects.toThrow(/current evopi-runtime with callable rlm\.run/);
	});

	it("rejects EVOPI_KERNEL_PYTHON with a legacy harness API", async () => {
		const overridePython = join(tempDir, "override-python");
		writeExecutable(
			overridePython,
			[
	...
	it("fails an invalid EVOPI_KERNEL_PYTHON without bootstrapping", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, []);
		process.env.EVOPI_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).rejects.toThrow(/EVOPI_KERNEL_PYTHON points to a Python missing/);
	});
});
