 ...
 * - ~/.evopi/agent/presets.json (global)
 * - <cwd>/.evopi/agent/presets.json (project-local)
 *
...
import type { ExtensionAPI, ExtensionContext } from "@evopi/pi-coding-agent";
import { DynamicBorder, getAgentDir } from "@evopi/pi-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@evopi/pi-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".evopi", "agent", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.evopi/agent/presets.json", "warning");
			return;
		}
