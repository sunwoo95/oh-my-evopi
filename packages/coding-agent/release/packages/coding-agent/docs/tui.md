> evopi can create TUI components. Ask it to build one for your use case.
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@evopi/pi-tui";
...
```typescript
import { Container, type Focusable, Input } from "@evopi/pi-tui";
...
Import from `@evopi/pi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@evopi/pi-tui";
```
...
evopi shows compact image metadata and does not emit terminal graphics for images.
...
```typescript
import { matchesKey, Key } from "@evopi/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@evopi/pi-tui";
...
```typescript
import { getMarkdownTheme } from "@evopi/pi-coding-agent";
import { Markdown } from "@evopi/pi-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@evopi/pi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@evopi/pi-coding-agent";
import { DynamicBorder } from "@evopi/pi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@evopi/pi-tui";
...
```typescript
import { BorderedLoader } from "@evopi/pi-coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@evopi/pi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@evopi/pi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@evopi/pi-tui";
...
Customize the inline working indicator shown while evopi is streaming a response.
...
// Restore evopi's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@evopi/pi-tui";
