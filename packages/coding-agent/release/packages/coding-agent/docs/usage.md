...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want evopi to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically as flat JSONL files under `~/.evopi/agent/sessions/`. Each session header records its working directory, which the session picker uses for project-scoped views.

```bash
evopi -c                  # Continue most recent session
evopi -r [path|id]        # Browse sessions or resume one directly
evopi --no-session        # Ephemeral mode; do not save
evopi --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `evopi agents`, `evopi list`, or `evopi attach <agent>` to find and reattach to running work. `evopi stop <agent>` stops one root agent, while `evopi shutdown` stops all workers and the local supervisor.
...
- `.evopi/agent/SYSTEM.md` for a project
- `~/.evopi/agent/SYSTEM.md` globally
...
evopi attach <agent>
evopi stop <agent>
evopi rename <agent> <name>
evopi send <agent> <message>
evopi schedule <list|add|cancel>
evopi status
evopi doctor [--fix]
evopi shutdown [--force]
...
See [evopi Packages](packages.md) for package sources and security notes.
...
Use `evopi session export <file> [output]` to export a session to HTML.
...
```bash
evopi --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `EVOPI_CODING_AGENT_DIR` | Override config directory; default is `~/.evopi/agent` |
...
| `EVOPI_KERNEL_PYTHON` | Use an existing Python environment with `evopi-runtime` instead of bootstrapping `~/.evopi/agent/kernel-venv` |
| `VISUAL`, `EDITOR` | External editor for Ctrl+G |

The remaining `PI_*` variables are compatibility names still read by the current runtime. They do not change the application name, command, or default `~/.evopi/agent` configuration path.

## Design Principles

evopi keeps the model-facing tool surface small while making the Python REPL runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and evopi packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

evopi preserves MIT attribution to pi-mono for its upstream lineage, but upstream Pi product claims and limitations do not describe the current evopi architecture.
