> evopi can create themes. Ask it to build one for your setup.
...
evopi loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.evopi/agent/themes/*.json`
- Project: `.evopi/agent/themes/*.json`
...
On first run, evopi detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.evopi/agent/themes
vim ~/.evopi/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, evopi reloads it automatically for immediate visual feedback.
...
evopi uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, evopi falls back to the nearest approximation.
