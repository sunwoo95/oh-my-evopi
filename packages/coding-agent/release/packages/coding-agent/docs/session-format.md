...
```
~/.evopi/agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.evopi/agent/sessions/`.

evopi also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, evopi uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@evopi/pi-coding-agent/dist/` and `node_modules/@evopi/pi-ai/dist/`.
...
### Base Message Types (from evopi AI)
...
### Extended Message Types (from evopi Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by evopi (legacy field name)
