...
Get a free API key at https://serper.dev, then run `/login` in evopi,
switch to **MCP Connections**, and choose **Serper (web search)** to paste it.
The key is stored in evopi and made available to this skill automatically.
...
- `EVOPI_WEBSEARCH_TIMEOUT` - HTTP timeout in seconds (default 45).
- `EVOPI_WEBSEARCH_NUM_RESULTS` - number of organic results to return (default 5).
