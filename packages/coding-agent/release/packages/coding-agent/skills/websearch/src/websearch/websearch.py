...
def _agent_dir() -> Path:
    """Resolve the evopi config dir the same way the runtime does (config.ts:526-530)."""
    raw = os.environ.get("EVOPI_CODING_AGENT_DIR") or str(Path.home() / ".evopi" / "agent")
    ...
    *,
    ...
    if not api_key:
        return (
            ...
            "  2. In evopi, run /login, switch to MCP Connections, choose \"Serper (web search)\", and paste the key.\n"
            "Do not ask the user to set environment variables. Once the key is saved, web search works automatically."
        )

    if timeout is None:
        timeout = _env_int("EVOPI_WEBSEARCH_TIMEOUT", 45)
    if num_results is None:
        num_results = _env_int("EVOPI_WEBSEARCH_NUM_RESULTS", 5)
