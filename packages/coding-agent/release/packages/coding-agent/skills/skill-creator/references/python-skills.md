...
A Python-backed skill is a regular markdown skill that also ships a Python package. evopi installs the package editable into the kernel venv (`~/.evopi/agent/kernel-venv` by default, `EVOPI_KERNEL_VENV` to override) and exposes it in the persistent Python kernel, so the agent can call it directly instead of shelling out.
...
- `rlm` and `tyro` are already present in the kernel venv. Do **not** declare `evopi-runtime` as a dependency: it is bundled with evopi, not published on PyPI, so declaring it breaks installs outside the kernel venv. The CLI entry point only works where the runtime is installed, i.e. inside the kernel venv.
...
- Declare every third-party package `run()` imports in `dependencies` — `pyproject.toml` is the source of truth. The one exception is `evopi-runtime` (see above).
...
- If the user sets `EVOPI_KERNEL_PYTHON`, evopi installs nothing — skills whose imports are missing there are disabled with a warning.
