> evopi can create skills. Ask it to build one for your use case.

# Skills

Skills are self-contained capability packages that evopi loads on demand. A skill provides specialized workflows, setup instructions, helper scripts, and reference documentation for specific tasks.

evopi implements the [Agent Skills standard](https://agentskills.io/specification), warning about violations but remaining lenient. It also supports Python-backed skills: a superset of markdown skills that install Python packages into the persistent Python kernel.
...
- [Creating Skills with evopi](#creating-skills-with-evopi)
...
- Built-in: `skills/` shipped with the evopi package (lowest precedence)

Discovery rules:
- In `~/.evopi/agent/skills/` and `.evopi/agent/skills/`, direct root `.md` files are discovered as individual skills
...
evopi ships with built-in skills that load by default:
...
```bash
export EVOPI_WEBSEARCH_TIMEOUT=45
export EVOPI_WEBSEARCH_NUM_RESULTS=5
```
...
```python
print(await websearch("latest evopi release"))
```
...
For project-level Claude Code skills, add to `.evopi/agent/settings.json`:
...
1. At startup, evopi scans skill locations and extracts names, descriptions, type, and file locations
...
For `web-search`, evopi exposes `web_search` in the Python REPL. If the module defines `run()`, the module is wrapped as an async callable:
...
Python skills are installed editable into the kernel venv during kernel setup. By default this is `~/.evopi/agent/kernel-venv`; set `EVOPI_KERNEL_VENV` to override it. If `pyproject.toml` changes, evopi rebuilds the kernel venv so dependency changes are picked up.

If you set `EVOPI_KERNEL_PYTHON`, evopi does not install packages into that environment. The Python must already have a current `evopi-runtime` and the default runtime packages installed. Missing Python skill imports are disabled with a warning and calling the skill raises a `RuntimeError`.
...
## Creating Skills with evopi

evopi ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:

```text
Create a project Python-backed skill named release-audit in
.evopi/agent/skills/release-audit. It should expose
...
1. **Scope:** use `.evopi/agent/skills/<name>/` for a project skill committed with the repository, or `~/.evopi/agent/skills/<name>/` for a personal skill.
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh evopi session after adding a Python-backed skill so kernel setup can install and import the package.
...
evopi validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
