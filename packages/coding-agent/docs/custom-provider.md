...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. evopi waits for the factory before startup continues, so the provider is available during interactive startup and to `evopi model list`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map evopi levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@evopi/pi-ai";
...
Credentials are persisted in `~/.evopi/agent/auth.json`:
...
interface ProviderModelConfig {
  ...
  /** Maps evopi thinking levels to provider/model-specific values; null marks a level unsupported. */
