> evopi can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to evopi's capabilities. Use it to embed evopi in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { AuthStorage, createAgentSession, ModelRegistry, SessionManager } from "@evopi/pi-coding-agent";
...
```bash
npm install @evopi/pi-coding-agent
```
...
```typescript
import { createAgentSession } from "@evopi/pi-coding-agent";
...
The `Agent` class (from `@evopi/pi-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.evopi/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.evopi/agent/extensions/`)
- Project skills:
  - `.evopi/agent/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.evopi/agent/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.evopi/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@evopi/pi-coding-agent";
...
// Default: uses ~/.evopi/agent/auth.json and ~/.evopi/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@evopi/pi-coding-agent";
...
- If you omit `tools`, evopi automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "@evopi/pi-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.evopi/agent/extensions/`, `.evopi/agent/extensions/`, and `settings.json` extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@evopi/pi-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@evopi/pi-coding-agent";
...
1. Global: `~/.evopi/agent/settings.json`
2. Project: `<cwd>/.evopi/agent/settings.json`
