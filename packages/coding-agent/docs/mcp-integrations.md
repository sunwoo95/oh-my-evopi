...
Connect external services (Linear, Notion, …) to evopi over the
[Model Context Protocol](https://modelcontextprotocol.io).

Consistent with evopi's single-tool design, MCP integrations are **not**
...
Credentials are stored once in `~/.evopi/agent/auth.json` under `mcp:<name>`.
...
agent) or the TUI. Both surfaces update only `~/.evopi/agent/settings.json`:

```bash
evopi mcp add remote --url https://mcp.example.com/mcp --bearer-token-env-var EXAMPLE_TOKEN
evopi mcp add local --cwd /absolute/path --env TOKEN=EXAMPLE_TOKEN -- node server.js --stdio
evopi mcp list
evopi mcp get remote
evopi mcp remove remote
```
...
are environment-variable references. Project `.evopi/agent/settings.json` MCP
...
- Generic MCP connections are kernel-local. Separate evopi sessions use
  separate connections even when they reference the same user setting.
- A custom `EVOPI_KERNEL_PYTHON` must include the current
  `evopi-runtime` dependencies.
