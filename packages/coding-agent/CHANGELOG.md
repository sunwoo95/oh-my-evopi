...
- **Subagent worktree isolation (NS-D1).** `await rlm('task', isolated=True)` runs the child in a detached `git worktree` under `~/.evopi/agent/worktrees/<repoHash>/<childId>` seeded with the parent's uncommitted changes when `subagent.worktree.mode` is `opt-in` or `always` (`EVOPI_SUBAGENT_WORKTREE`); the child's delta is captured to `<child session dir>/worktree.patch`, applied to the parent checkout (`merge: patch-apply`, index untouched; conflicts keep the patch and report its path) and the worktree is removed. Isolated children close after the task (not resumable); the parent gets an 'RLM child status' notice. Default mode `off` keeps the spawn path byte-identical (`isolated=True` then fails with a hint). Added `/worktree [list|prune [--all] [--dry-run]]` and a best-effort startup prune of worktrees whose owner exited.
- **Edit checkpoints and `/rewind` (NS-D4).** Files written by the kernel `edit` skill (cell and `!edit` shell form) and by `hashline_edit` are snapshotted before the write for persistent sessions under `session-artifacts/<id>/edit-checkpoints/` (content-addressed, deduplicated, removed with the session; retention `editCheckpoint.maxRecords` 200 / `maxTotalBytes` 64 MiB / `maxFileBytes` 4 MiB). `/rewind list`, `/rewind <N|seq> [--with-conversation] [--restart-kernel] [--force]` and an interactive picker restore files (drift outside the tracked editors is refused without `--force`); the model receives an `edit_rewind_notice`. Default on; opt out with `editCheckpoint.enabled: false` or `EVOPI_EDIT_CHECKPOINT=off`; never active under `--no-session`.
- **Steering and abort (D2 emulation, frozen agent loop untouched).** Steering messages sent mid-run are injected at the loop's own turn boundary instead of stopping and restarting the run (`EVOPI_STEER_MODE=restart` restores the old behaviour); preserved steering/follow-up items auto-resume after Esc/Ctrl+C (`EVOPI_STEER_AUTO_RESUME=off` keeps manual resume). Tools may declare `interruptible` to be hard-aborted by a queued steering message, every tool receives a cooperative `ctx.steeringSignal`, and sequential tools not yet started when a steer is queued return a "Skipped due to queued user message" result. Aborted-turn labels are unified. The two-stage upstream proposal for prime lives in `docs/design/upstream-proposals/prime-d2.md`.
- **Cell timeout UX (A4).** `/kernel` shows the effective Python cell cap (source: chat/env/settings/default) and the running cell's elapsed time; `/kernel timeout <ms|Ns|Nm|Nh|off> [--global]` changes it immediately — re-arming the running cell when possible — and is recorded in the session log and restored on resume (`--global` also writes `kernel.cellTimeoutMs`). The `ipython` tool warns once at 80% of the cap (streamed line + TUI toast) and appends a one-line note to results of cells that crossed 80%; a clean timeout interrupt raises a warning toast, a kernel restart an error toast. Cells below 80% and cap-off sessions stay byte-identical. Daemon protocol schema revision 26.
- **Harness consolidation (B3/B4, evo layer).** grounded-refine gains a success-without-experience branch: a pass signal (`pass|passed|ok|success|succeeded|solved`) with zero recall hits in the session runs one "success note" round per task. Refinement proposals accept `add`/`remove` aliases and an explicit `{"action":"skip"}` recorded on `skippedEdits`. When `harness.capPerKind` / `EVOPI_HARNESS_CAP_PER_KIND` resolves (evo on defaults to 80) the planner prompt gains a consolidation addendum with an LFU-ordered overview and applies enforce the cap: prompt/memory overflow is LFU-evicted as rollback-able deletes, creates into a full skill/subagent kind are rejected ("kind at capacity; REMOVE first"), progress-ledger entries are excluded. Fixed planned UPDATE/DELETE edits failing when a kernel recall only bumped `usage_count` during planning.
- **Release automation (E2).** Pushing a plain `vX.Y.Z` tag is now the canonical release path: `release.yml` refuses pre-release tags, version mismatches with `package.json`, and already-published versions, publishes only the `stable` channel under one concurrency group, and offers a `dry_run` dispatch that uploads the assembled site as an artifact. Removed the inherited `build-binaries.yml`. `scripts/release.mjs` skips the npm registry unless `--npm-publish` / `EVOPI_RELEASE_NPM_PUBLISH=1`, refuses to run off `main`, and pushes only the release tag. Added `scripts/compare-release-artifacts.mjs` and `docs/release.md`. `bundle.mjs` honors `EVOPI_BUILD_ID` for reproducible bundles. CI's Python runtime job now points at `evopi-runtime`.
- **Seminar deck (E1).** Added the "안전 기본값 — v0.10.0 · v0.11.0" slide (22) and relabeled the resolved risk cards; deck 42→43 slides, stale figures refreshed.
- **Child worktree doctrine.** Isolated children receive an `[isolation]` paragraph in their system prompt describing the worktree contract; `RlmSpawnHandle.worktree` and `CreateRlmSubagentRuntimeOptions.cwd/worktree` were added so every runtime host honors the child cwd.
...
- **Permission gate no longer flags every recursive `rm` (A3).** `rm -rf ./dist`, `rm -rf node_modules`, `cd build && rm -rf out` and absolute paths under the session cwd pass without a prompt (and are no longer blocked in no-UI runs). Still flagged: `/`, `~`/`$HOME`, a bare `*` or `/*`, `..` traversal leaving the cwd, absolute paths outside the cwd (including after an earlier `cd`), `--no-preserve-root` and anything under `sudo`. Every other destructive pattern is unchanged. New `permissionGate.mode` (`block`|`warn`|`off`; `EVOPI_PERMISSION_GATE` still wins) and `permissionGate.allow` (regex whitelist that bypasses the gate; invalid entries are ignored with a one-time warning) settings, honored from both `~/.evopi/agent/settings.json` and the project `.evopi/agent/settings.json`.
- **Gate decisions are logged (A5).** Each `allowed-by-whitelist` / `warned` / `blocked` / `confirmed-by-user` / `denied-by-user` decision is appended to the session log as a `permission_gate` entry carrying the hazard kind, tool, mode and the first 16 hex chars of the command's SHA-256 — never the command text. Logging failures cannot affect the decision.
- **Kernel env allowlist mode (A2).** `kernel.envPolicy: "allowlist"` (or `EVOPI_KERNEL_ENV_POLICY=allowlist`) passes only a fixed safe set (`PATH`, `HOME`, locale, `TERM`, temp dirs, `XDG_*`, `EVOPI_*`, Python tooling, CA bundles, proxies) plus `kernel.envAllow` names (`"MYCO_*"` = prefix) into the Python kernel, so unknown `*_API_KEY`/`*_TOKEN` variables are withheld. Provider credentials stay withheld in both modes unless `EVOPI_KERNEL_INHERIT_SECRETS=1`. The default `denylist` policy is byte-identical to 0.10.0; the allowlist diagnostic lists at most 12 withheld names then `+N more`. The settings reach `ReplKernelManager` via new `envPolicy`/`envAllow` options; the env override is effective everywhere.
- **oh-my-pi extension compatibility.** Extensions written for oh-my-pi load unchanged: `@oh-my-pi/pi-coding-agent`, `pi-agent-core`, `pi-tui`, `pi-ai`, `pi-ai/oauth` resolve to the bundled evopi modules, and the ExtensionAPI gains `pi.zod` (a Zod-v4-style builder backed by TypeBox — the result is a real TSchema usable as `parameters`), `pi.typebox` and `pi.logger`. `refine`/`transform`/`catch` do not change the wire schema; omp-only `arktype`, service-tier and renderer APIs are not provided.
- **Extension load failures are visible in print/json mode.** A broken `-e` file or a failing extension in `~/.evopi/agent/extensions` now prints `Warning: Failed to load extension "…": …` on stderr and the session continues without it. Previously the daemon's create reply dropped these diagnostics, so nothing was ever shown.
- **Release scorecard automation.** `npm run self-eval` collects the SELF-EVAL metrics (types, lint, tests, bundle size, kernel secret filter, gate coverage, timeouts, startup time, shellcheck, forbidden literals) as JSON; `--baseline` compares against `eval/self-eval/<version>.json`. `scripts/release.mjs` now verifies every workspace version after `npm version` instead of trusting its exit code.
...
- Added the RLM harness system prompt and `evopi-runtime` bridge so IPython code can call `rlm.run` to spawn recursive child agent sessions.
- Added automatic IPython runtime bootstrap with uv-managed Python, `ipykernel`, and `evopi-runtime`.
