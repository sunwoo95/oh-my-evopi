...
/**
 * `daemon ps` discovers every evopi daemon on the machine, not just the
 * one on a single socket. Discovery has two sources merged by socket path:
 *
 *  1. The OS list of listening unix sockets owned by a evopi process
...
/** Parse `ss -lxp` output into the evopi daemons listening on unix sockets. */
	...
	return daemons.map((daemon): ReapAction => {
		...
		if (daemon.status === "unreachable") {
			if (!force || daemon.pid === undefined) {
				return { kind: "skip", daemon, reason: 'unreachable; use "evopi shutdown --force" to stop it' };
			}
				...
				JSON.stringify(
					{
						...
						failed: daemons.map(({ socketPath }) => ({
							socketPath,
							reason: 'confirmation required; use "evopi shutdown --force --json"',
						})),
					},
					null,
					2,
