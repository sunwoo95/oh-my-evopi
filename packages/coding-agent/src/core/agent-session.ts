...
export class AgentSession {
	...
	private async _applyRefine(
		...
		try {
			...
			try {
				this.sessionManager.appendCustomEntry("evopi.refinement", result);
			} catch (error) {
	...
	private _addWebsearchKeyEnv(env: Record<string, string>): void {
		if (this._agentDir) {
			env.EVOPI_CODING_AGENT_DIR = this._agentDir;
		}
	...
	private _createEphemeralRlmSessionDir(): string {
		this._rlmSessionDir = mkdtempSync(join(tmpdir(), "evopi-rlm-"));
		return this._rlmSessionDir;
	}
