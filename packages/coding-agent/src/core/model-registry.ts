...
import { registerOAuthProvider, resetOAuthProviders } from "@evopi/pi-ai/oauth";
...
const OpenAICompletionsCompatSchema = Type.Object({
	...
	invocationsPath: Type.Optional(Type.Boolean()),
});
 ...
 * than an error. evopi's own package version is far below the Codex CLI's version line, so it must
 ...
 *    explicit, not fetched, so an unlisted model does not exist for evopi at all.
