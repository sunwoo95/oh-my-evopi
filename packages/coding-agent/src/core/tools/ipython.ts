...
import type { ImageContent, TextContent } from "@evopi/pi-ai";
...
try:
    ...
    _EVOPI_RLM_IMPORT_ERROR = str(_prime_agent_rlm_error)

    class _PrimeAgentMissingRlm:
        def _raise_missing(self):
            raise RuntimeError(
                "evopi-runtime is not installed in this kernel. "
                "Remove ~/.evopi/agent/kernel-venv so evopi can rebuild it, or set "
                "EVOPI_KERNEL_PYTHON to a kernel environment with evopi-runtime installed. "
                f"Import error: {_EVOPI_RLM_IMPORT_ERROR}"
            )
...
for _prime_agent_skill_name in ${JSON.stringify(importNames)}:
    try:
        ...
        _EVOPI_SKILL_IMPORT_ERRORS[_prime_agent_skill_name] = str(_prime_agent_skill_error)
...
export interface IpythonToolOptions {
	/** Python override. Must have evopi-runtime installed. */
	python?: string;
