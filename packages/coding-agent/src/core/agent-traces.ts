...
export async function getPrimeAgentTraceCredential(
	...
	const traceEnvKey = stringEnv("EVOPI_TRACES_API_KEY");
	if (traceEnvKey) {
		return { apiKey: traceEnvKey, source: "environment", label: "EVOPI_TRACES_API_KEY" };
	}
	...
	const traceKey = await authStorage.getApiKey(EVOPI_TRACES_PROVIDER_ID, { includeFallback: false });
	if (traceKey) {
		return { apiKey: traceKey, source: "stored", label: "evopi Traces credential" };
	}
