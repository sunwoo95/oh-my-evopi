...
import type { ThinkingLevel } from "@evopi/pi-agent-core";
import type { Model, ServiceTier } from "@evopi/pi-ai";
...
import { createGroundedRefineExtension } from "./extensions/builtin/grounded-refine.js";
import { createHerdrAgentStateExtension } from "./extensions/builtin/herdr-agent-state.js";
import { createPermissionGateExtension } from "./extensions/builtin/permission-gate.js";
...
export async function createAgentSessionServices(
	...
	// stays false and prime's stock refinement path is untouched. See DECISIONS M11.
	...
	if (
		...
		diagnostics.push({
			type: "info",
			message:
				"evopi sends pseudonymous usage and performance metrics without prompts, responses, tool content, file paths, or repository data. Disable this with telemetry.enabled=false, EVOPI_TELEMETRY=0, DO_NOT_TRACK=1, or offline mode.",
		});
