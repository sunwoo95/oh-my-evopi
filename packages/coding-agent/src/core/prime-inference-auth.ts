...
import type { OAuthAuthInfo } from "@evopi/pi-ai";

export const PRIME_INFERENCE_PROVIDER_ID = "prime-inference";
export const PRIME_INFERENCE_PROVIDER_NAME = "Prime Inference";
export const EVOPI_TRACES_PROVIDER_ID = "evopi-traces";
export const EVOPI_TRACES_PROVIDER_NAME = "evopi Traces";
...
export function resolvePrimeAgentTracesBaseUrl(baseUrl?: string): string {
	return normalizeBaseUrl(baseUrl ?? stringEnv("EVOPI_TRACES_BASE_URL"));
}
...
function resolvePrimeAgentTracesChallengeConfig(config: PrimeCliConfig): PrimeChallengeConfig {
	return {
		...
		frontendUrl: stringEnv("EVOPI_TRACES_BASE_URL") ? config.frontendUrl : DEFAULT_PRIME_FRONTEND_URL,
	};
}
...
export async function loginPrimeAgentTraces(
	...
	if (config.apiKey) {
		...
		callbacks.onProgress?.(
			`Existing Prime CLI key cannot upload evopi traces (${formatAccessFailure(access)}). Starting browser login...`,
		);
	} else {
		callbacks.onProgress?.("No Prime CLI API key found. Starting browser login...");
	}
	...
	throwIfCancelled(callbacks.signal);
	callbacks.onProgress?.("Checking evopi trace access...");
	...
	if (!access.ok) {
		throw new Error(`Prime API key does not have evopi trace access (${formatAccessFailure(access)})`);
	}
