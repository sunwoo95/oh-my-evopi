import type { ServiceTier, Transport } from "@evopi/pi-ai";
...
export interface EvoSettings {
	// evo-layer (grounded-refine) toggle. Unset = prime stock behavior (autoRefine on,
	// grounded-refine not loaded). Explicit false = pure A/B control arm (autoRefine off).
	// Explicit true = grounded arm (extension loaded + autoRefine on). Overridden by the
	// EVOPI_EVO env var ("on"|"off"). See DECISIONS M11.
	enabled?: boolean;
}
...
export interface Settings {
	...
	bundledSkills?: BundledSkillsSettings; // Configure built-in skills shipped with evopi
	enableBuiltinSkills?: boolean; // default: true - load built-in skills shipped with evopi
...
export class SettingsManager {
	...
	/**
	 * Resolve the evo-layer state. `EVOPI_EVO` ("on"|"off") env var takes
	 * precedence over the `evo.enabled` setting. Returns undefined when neither
	 * is set (prime stock behavior). See DECISIONS M11.
	 */
	resolveEvoEnabled(): boolean | undefined {
		const raw = (process.env.EVOPI_EVO ?? "").trim().toLowerCase();
	...
	getAutoRefineSettings(): { enabled: boolean; turnInterval: number; compact: boolean; cooldownMs: number } {
		...
		// evo explicitly off maps to the pure control arm (autoRefine disabled); evo unset
		// leaves prime's default (enabled unless the user opted out).
		const evo = this.resolveEvoEnabled();
		return {
			enabled: evo === false ? false : (this.settings.autoRefine?.enabled ?? true),
