...
const RUNTIME_REQUIREMENT = "evopi-runtime";
...
function ensureKernelPythonKey(pythonSkills: readonly BootstrapPythonSkill[]): string {
	return [
		process.env.EVOPI_KERNEL_PYTHON ?? "",
		process.env.EVOPI_KERNEL_VENV ?? "",
		process.env.HOME ?? "",
...
export function getKernelVenvDir(): string {
	const override = process.env.EVOPI_KERNEL_VENV;
...
async function resolveWritableKernelVenvDir(): Promise<string> {
	...
	try {
		...
		if (process.env.EVOPI_KERNEL_VENV) {
		...
		try {
			...
			throw new Error(
				`couldn't create kernel venv directory at ${primary} or ${fallback}; set EVOPI_KERNEL_PYTHON to a python with a current evopi-runtime installed. ${errorMessage(fallbackError)}`,
			);
		}
	}
}
...
async function ensureUv(options: EnsureKernelPythonOptions): Promise<string> {
	...
	if (!shouldInstallUv) {
		throw new Error(
			`uv is required to set up the Python kernel. Install uv yourself: ${UV_INSTALL_COMMAND}, ` +
				"or set EVOPI_INSTALL_UV=1 to let evopi run that installer.",
		);
	}
	...
	try {
		...
		throw new Error(
			`couldn't install uv from astral.sh; install it yourself: ${UV_INSTALL_COMMAND}, then re-run evopi. ${errorMessage(error)}`,
		);
	}
...
async function confirmUvInstall(): Promise<boolean> {
	if (process.env.EVOPI_INSTALL_UV === "0") return false;
	if (!stdin.isTTY || !stderr.isTTY) return false;

	const rl = createInterface({ input: stdin, output: stderr });
	try {
		const answer = (await rl.question("evopi needs uv to set up Python. Install uv from astral.sh now? [Y/n] "))
			.trim()
			.toLowerCase();
...
function runtimeCandidateDirs(): string[] {
	...
	// dist/evopi-runtime is listed first deliberately: it is the only path stable
	...
	return [
		path.join(getPackageDir(), "dist", "evopi-runtime"),
		path.resolve(moduleDir, "..", "..", "evopi-runtime"),
		path.resolve(moduleDir, "..", "..", "..", "..", "..", "evopi-runtime"),
	];
}
...
function formatBootstrapFailure(error: unknown): Error {
	return new Error(
		`Failed to set up the Python kernel runtime. ${errorMessage(error)}\n` +
			"First-time setup needs internet to install uv, Python, evopi-runtime, and default Python packages; once set up, evopi runs offline. " +
			"Set EVOPI_KERNEL_PYTHON to a Python with a current evopi-runtime and default Python packages installed to skip auto-bootstrap.",
	);
}
...
async function ensureKernelPythonUncached(
	...
	const override = process.env.EVOPI_KERNEL_PYTHON;
	if (override) {
		const python = path.resolve(expandHome(override));
		const missing: string[] = [];
		if (!(await hasPrimeAgentRuntime(python))) {
			missing.push(
				"a current evopi-runtime with callable rlm.run, rlm.host_request, and explicit harness CRUD methods",
			);
		}
		if (missing.length === 0) {
		...
		if (missing.length === 0 && pythonSkills.length > 0) {
			...
			if (missingPythonSkills.length > 0) {
				reportProgress(
					options,
					`Warning: Python skills unavailable in EVOPI_KERNEL_PYTHON and will be disabled: ${missingPythonSkills.join(", ")}`,
				);
			}
		}
		if (missing.length === 0) return python;
		throw new Error(`EVOPI_KERNEL_PYTHON points to a Python missing ${missing.join(" and ")}: ${python}`);
	}
