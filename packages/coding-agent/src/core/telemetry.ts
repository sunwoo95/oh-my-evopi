...
import type { AssistantMessage, Usage } from "@evopi/pi-ai";
...
export function isTelemetryEnabled(settingsManager: SettingsManager): boolean {
	...
	const override = parseBooleanOverride(process.env.EVOPI_TELEMETRY);
...
export class TelemetryClient implements TelemetrySink {
	...
	constructor(private readonly options: TelemetryClientOptions) {
		this.endpoint = options.endpoint ?? process.env.EVOPI_TELEMETRY_ENDPOINT ?? DEFAULT_TELEMETRY_ENDPOINT;
