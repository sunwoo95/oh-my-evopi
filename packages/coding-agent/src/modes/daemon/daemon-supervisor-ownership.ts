...
const DAEMON_SUPERVISOR_REGISTRY_DIR_ENV = "EVOPI_INTERNAL_DAEMON_SUPERVISOR_REGISTRY_DIR";
...
function defaultDaemonSupervisorRegistryDir(environment: NodeJS.ProcessEnv = process.env): string {
	return environment[DAEMON_SUPERVISOR_REGISTRY_DIR_ENV] ?? join(homedir(), `.${APP_NAME}`, "supervisor-owners");
}
 ...
 * from before the home-dir registry move may still be running; gated off whenever the
