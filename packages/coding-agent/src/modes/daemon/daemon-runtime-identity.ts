...
export const EVOPI_BUILD_ID_ENV = "EVOPI_BUILD_ID";
export const EVOPI_LAUNCHER_PATH_ENV = "EVOPI_LAUNCHER_PATH";
...
export function getDaemonRuntimeIdentity(environment: NodeJS.ProcessEnv = process.env): DaemonRuntimeIdentity {
	...
	const launcher = environment[EVOPI_LAUNCHER_PATH_ENV];
	return {
		buildId: environment[EVOPI_BUILD_ID_ENV] ?? bundledBuildId() ?? `release-${VERSION}`,
