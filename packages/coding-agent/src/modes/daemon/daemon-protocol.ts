...
import type { ImageContent, ServiceTier, TextContent, Transport } from "@evopi/pi-ai";
import type {
...
export const DAEMON_PROTOCOL_NAME = "evopi.daemon";
...
export function collectDaemonLaunchEnv(source: NodeJS.ProcessEnv = process.env): Record<string, string> {
	...
	for (const [key, value] of Object.entries(source)) {
		if (value !== undefined && !key.startsWith("EVOPI_INTERNAL_")) {
			env[key] = value;
		}
	}
	return env;
