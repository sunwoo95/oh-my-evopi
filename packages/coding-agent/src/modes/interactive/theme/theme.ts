...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		const evopiPath = path.join(themesDir, "evopi.json");
		const darkPath = path.join(themesDir, "dark.json");
		const lightPath = path.join(themesDir, "light.json");
		BUILTIN_THEMES = {
			evopi: JSON.parse(fs.readFileSync(evopiPath, "utf-8")) as ThemeJson,
...
function getDefaultTheme(): string {
	// evopi brand is dark-first; only fall back to light when the terminal is light.
	return detectTerminalBackground() === "light" ? "light" : "evopi";
}
...
const THEME_KEY = Symbol.for("@evopi/pi-coding-agent:theme");
...
export function getSettingsListTheme(): import("@evopi/pi-tui").SettingsListTheme {
	return {
