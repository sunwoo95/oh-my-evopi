...
import { getOAuthProviders } from "@evopi/pi-ai/oauth";
import {
	type Component,
	Container,
	type Focusable,
...
const EVOPI_LOGO_LINES = EVOPI_LOGO.split("\n");
const EVOPI_LOGO_WIDTH = EVOPI_LOGO_LINES.reduce((max, line) => Math.max(max, visibleWidth(line)), 0);
...
class PrimeLoginHeader implements Component {
	...
	render(width: number): string[] {
		const safeWidth = Math.max(1, width);
		const logoWidth = Math.min(EVOPI_LOGO_WIDTH, safeWidth);
		const logoLines = EVOPI_LOGO_LINES.map((line) => {
			const paddedLogoLine = line + " ".repeat(Math.max(0, EVOPI_LOGO_WIDTH - visibleWidth(line)));
