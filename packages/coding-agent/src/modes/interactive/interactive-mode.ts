...
import { BUILTIN_MCP_CATALOG } from "@evopi/pi-ai/mcp";
import type {
	AutocompleteItem,
...
import { EVOPI_LOGO } from "../../themes/evopi-logo.js";
...
export class BrandSplashHeader implements Component {
	...
	constructor(
		...
		this.logoRaw = (options.logo ?? EVOPI_LOGO).split("\n");
			...
			const selector = new SettingsSelectorComponent(
				{
					...
					currentTheme: this.settingsManager.getTheme() || "evopi",
			...
			if (result.changed && result.serverChange) {
				...
				const successMessage =
					verb === "removed"
						...
						: usesOAuth
							? `${verb === "replaced" ? "Replaced" : "Added"} MCP server "${name}" (${transport}). ${hasMcpProviderRefresh ? "Run" : "Restart evopi, then run"} /mcp login ${name} to connect.`
		...
		switch (result.status) {
			...
			case "failed":
				if (result.statusCode === 404) {
					return "Trace upload endpoint was not found. The platform API may not be deployed yet, or EVOPI_TRACES_BASE_URL points at the wrong API.";
				}
