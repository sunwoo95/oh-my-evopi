...
export class ProviderAuthFlows {
	...
	private async completePrimeAgentTracesLogin(apiKey: string, closeDialog: () => void): Promise<AuthenticationResult> {
		this.host.modelRegistry.authStorage.set(EVOPI_TRACES_PROVIDER_ID, {
			type: "api_key",
			key: apiKey,
		});
	...
	async runPrimeAgentTracesLogin(): Promise<AuthenticationResult> {
		...
		try {
			...
			if (result.source === "manual") {
				browserAbort.abort();
				dialog.showProgress("Checking evopi trace access...");
				...
				if (!access.ok) {
					const status = access.status === undefined ? "" : `HTTP ${access.status}: `;
					throw new Error(`Prime API key does not have evopi trace access (${status}${access.message})`);
				}
			}
			...
			const errorMsg = error instanceof Error ? error.message : String(error);
			if (!dialog.signal.aborted && errorMsg !== "Login cancelled") {
				this.host.showError(`Failed to login to ${EVOPI_TRACES_PROVIDER_NAME}: ${errorMsg}`);
				return { status: "failed" };
			}
			return { status: "cancelled" };
		} finally {
