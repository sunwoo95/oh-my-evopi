...
import type { ImageContent } from "@evopi/pi-ai";
...
import { EVOPI_META_NAMESPACE, type PrimeAgentAutonomousMeta, primeAgentMeta } from "./acp-meta.js";
...
class AcpUpdateProducer {
	...
	async publish(
		...
		const priorPrimeMeta =
			priorMeta[EVOPI_META_NAMESPACE] && typeof priorMeta[EVOPI_META_NAMESPACE] === "object"
				? (priorMeta[EVOPI_META_NAMESPACE] as Record<string, unknown>)
				: {};
...
/**
 * Split ACP prompt blocks into the text and images evopi accepts.
 *
...
export async function runAcpModeWithConnection(
	...
	const handle = acp
		...
		.onRequest("initialize", async () => ({
			...
			agentInfo: { name: "evopi", title: "evopi", version: VERSION },
			// Advertise evopi extras under a namespaced key: ACP reserves
		...
		.onRequest("session/new", async (ctx: any) => {
			...
			try {
				...
				// evopi's cwd is fixed at startup by the session it was launched
				...
				// Subscribe for the session lifetime, not per prompt turn: evopi
			...
			try {
				...
				if (!abort.signal.aborted) {
					...
					if (pending.turnFailure) {
						throw new Error(`evopi turn failed: ${pending.turnFailure}`);
					}
					terminalStatus = pending.status ?? status;
				}
				if (failure) throw new Error(`evopi turn failed: ${failure}`);
				return {
