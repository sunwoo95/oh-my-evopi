import type { AssistantMessageEvent } from "@evopi/pi-ai";
...
/**
 * Translate evopi session events into ACP `session/update` payloads.
 *
 * Kept as a pure function so the mapping is testable without a live ACP client
 * or a running agent. Returning an array lets one evopi event fan out to
...
/** evopi's model-facing tool is the Python REPL; bash is the secondary escape hatch. */
...
function startAssistantMessage(state: AcpEventMappingState): string {
	...
	state.activeAssistantMessageId = `evopi-assistant-${sequence}`;
...
export function acpUpdatesForSessionEvent(
	...
	switch (event.type) {
		...
		// evopi concepts with no ACP counterpart. They are still part of a
...
const BASH_TOOL_CALL_PREFIX = "evopi-bash";
