/**
 * Namespaced `_meta` payloads for evopi capabilities that ACP has no
 ...
 * clients ignore these keys; a evopi-aware client (or the verifiers
 * harness) reads them. Never add non-standard fields to an ACP object root.
 */

/** Reverse-domain namespace for every evopi `_meta` payload. */
export const EVOPI_META_NAMESPACE = "ai.primeintellect.evopi";
...
export interface PrimeAgentCwdMeta {
	...
	requested: string;
	/** The cwd evopi is actually running in, fixed at startup. */
	actual: string;
}
 ...
 * strictly increases for every update evopi publishes.
 */
...
/** Wrap a evopi payload in its reverse-domain `_meta` envelope. */
export function primeAgentMeta(payload: PrimeAgentSessionMeta): Record<string, unknown> {
	return { [EVOPI_META_NAMESPACE]: payload };
}
