...
describe("Copilot Claude via Anthropic Messages", () => {
	...
	it("includes interleaved-thinking beta when reasoning is enabled", async () => {
		// claude-sonnet-4.6+ has adaptive thinking built in and skips this legacy beta
		// header (see supportsAdaptiveThinking in anthropic.ts), so this test needs a
		// model that predates that to actually exercise the beta-injection path.
