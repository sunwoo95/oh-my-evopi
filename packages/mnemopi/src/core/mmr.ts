import { mmrRerankIndices } from "../native.js";
...
/**
 * Whether `s` contains no lone surrogates. Uses the ES2024
 * `String.prototype.isWellFormed` when available (Node ≥20), else a regex
 * fallback so the `lib: ES2022` build type-checks. Lone-surrogate contents must
 * stay on the TS path: the native N-API boundary converts them to U+FFFD, which
 * would merge distinct tokens.
 */
function isWellFormed(s: string): boolean {
	const fn = (s as { isWellFormed?: () => boolean }).isWellFormed;
...
export function mmrRerank<T extends MmrResult>(
	...
	// is preserved. A `null` native result (addon unavailable) falls through to
	// the greedy TS loop below.
	if (
