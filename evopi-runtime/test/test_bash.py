...
class BashTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_status_pipe_survives_high_fds_and_strict_posix_shell(self):
        ...
        if os.path.exists("/bin/dash"):
            with mock.patch.dict(os.environ, {"EVOPI_BASH_SHELL": "/bin/dash"}):
    ...
    async def test_env_prefix_and_journal(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.dict(
                os.environ,
                {
                    "EVOPI_BASH_COMMAND_PREFIX": "echo prefixed",
                    "EVOPI_INTERNAL_ORPHAN_PROCESS_JOURNAL": journal,
                    "EVOPI_KERNEL_OWNER_PID": str(os.getpid()),
                },
            ):
    ...
    async def test_windows_without_bash_raises_teaching_error(self):
        ...
        with mock.patch.object(bash_module, "_IS_POSIX", False):
            with mock.patch.object(
                ...
                with self.assertRaisesRegex(RuntimeError, "EVOPI_BASH_SHELL"):
                    bash_module._shell()
    ...
    async def test_sentinel_like_output_and_echoed_wrapper_do_not_truncate(self):
        ...
        raw_lookalike = "\x1eevopi-complete:not-this-invocation\x1f"
        command = (
            "set -x\n"
            "printf '\\036evopi-complete:not-this-invocation\\037'\n"
    ...
    async def test_relative_bash_shell_override_rejected(self):
        with mock.patch.dict(os.environ, {"EVOPI_BASH_SHELL": "bash"}):
    ...
    async def test_windows_spawn_creates_child_inside_job(self):
        ...
        self.enterContext(mock.patch.dict(os.environ, {"EVOPI_BASH_SHELL": "/bin/sh"}))
        with mock.patch.object(bash_module, "_IS_POSIX", False):
    ...
    async def test_journal_configured_but_unwritable_kills_child_and_raises(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.dict(
                os.environ,
                {
                    "EVOPI_INTERNAL_ORPHAN_PROCESS_JOURNAL": tmp,  # a directory: open fails
                    "EVOPI_KERNEL_OWNER_PID": str(os.getpid()),
                },
            ):
    ...
    async def test_journal_bad_owner_pid_rejects(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.dict(
                ...
                {
                    "EVOPI_INTERNAL_ORPHAN_PROCESS_JOURNAL": journal,
                    "EVOPI_KERNEL_OWNER_PID": "notanint",
                },
            ):
                with self.assertRaises(RuntimeError):
                    bash("echo hi")
