...
class ReplTest(unittest.TestCase):
    ...
    def test_emit_display(self):
        payloads = {
            "application/vnd.evopi.diff+json": {
                "path": "/tmp/file.py",
                "old_str": "a",
                "new_str": "b",
                "start_line": 3,
            },
            "application/vnd.evopi.attachment+json": {
                "mime_type": "image/png",
                "data": "aGVsbG8=",
                "path": "/tmp/img.png",
            },
            "application/vnd.evopi.agent-message+json": {
                "id": "agentmsg_1",
                "message": "hi",
...
class OwnerWatchdogTest(unittest.TestCase):
    def test_owner_watchdog_exits_busy_runtime(self):
        ...
        try:
            repl = ReplProcess(env={"EVOPI_KERNEL_OWNER_PID": str(owner.pid)})
    ...
    def test_resolve_owner_pid_falls_back_to_ppid(self):
        ...
        with mock.patch.dict(os.environ, {"EVOPI_KERNEL_OWNER_PID": "4242"}):
            self.assertEqual(repl_module._resolve_owner_pid(), 4242)
        for raw in (None, "", "garbage", "0", "-7"):
            env = {} if raw is None else {"EVOPI_KERNEL_OWNER_PID": raw}
            with mock.patch.dict(os.environ, env, clear=False):
                if raw is None:
                    os.environ.pop("EVOPI_KERNEL_OWNER_PID", None)
