...
class NotEnabled(RuntimeError):
    ...
    def __init__(self, server: str):
        ...
        super().__init__(
            ...
            f"Tell the user to run `/mcp login {server}` in evopi to connect it. "
...
def _agent_dir() -> Path:
    """Resolve the evopi config dir the same way the rest of the runtime does."""
    raw = (
        os.environ.get("EVOPI_CODING_AGENT_DIR")
        or os.environ.get("PI_CODING_AGENT_DIR")
        or str(Path.home() / ".evopi" / "agent")
    )
