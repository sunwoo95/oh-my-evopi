...
_STATUS_FD = 9
_OUTPUT_FD = 8
_COMPLETION_PREFIX = b"\x1eevopi-complete:"
...
def _shell() -> str:
    ...
    override = os.environ.get("EVOPI_BASH_SHELL")
    if override:
        if not os.path.isabs(override):
            raise ValueError("EVOPI_BASH_SHELL must be an absolute path")
        return override
    if not _IS_POSIX:
        # Never consult PATH on Windows: a repo-controlled PATH could supply
        # the shell. The host injects EVOPI_BASH_SHELL when one exists.
        raise RuntimeError(
            "bash() needs EVOPI_BASH_SHELL set to the absolute path of a "
    ...
    # always injects EVOPI_BASH_SHELL (an absolute path) when a shell exists.
    shell = shutil.which("bash")
    return shell or "/bin/sh"


def _with_prefix(command: str) -> str:
    prefix = os.environ.get("EVOPI_BASH_COMMAND_PREFIX")
...
def _status_script(command: str, completion_a: str, completion_b: str) -> str:
    ...
    return (
        ...
        f"read -r _evopi_gate <&{_STATUS_FD} || exit 127\n"
        "{\n"
        f"{command}\n"
        f"}} {_OUTPUT_FD}>&- {_STATUS_FD}>&-\n"
        "__evopi_status=$?\n"
        "\\set +x\n"
        f"{emit} '\\036evopi-complete:%s%s\\037' "
        f"'{completion_a}' '{completion_b}' >&{_OUTPUT_FD} || exit \"$__evopi_status\"\n"
        f"{emit} '%s\\n' \"$__evopi_status\" >&{_STATUS_FD}\n"
...
def _record_journal(pid: int, active: bool) -> bool:
    ...
    path = os.environ.get("EVOPI_INTERNAL_ORPHAN_PROCESS_JOURNAL")
    owner = os.environ.get("EVOPI_KERNEL_OWNER_PID")
